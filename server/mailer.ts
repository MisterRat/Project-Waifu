...
export async function sendAccountLockedEmail(adminEmail: string, userEmail: string, unlockUrl: string) {
  const smtp = await getSmtpConfig();

  if (!smtp || !smtp.host || !adminEmail) {
    console.log(`[Mailer] Account locked for ${userEmail}. Admin email not configured or SMTP disabled. Unlock link: ${unlockUrl}`);
    return { sent: false, unlockUrl };
  }
  ...
  try {
    ...
    await transporter.sendMail({
      from: smtp.fromEmail || `"Project Waifu Security" <no-reply@${smtp.host}>`,
      to: adminEmail,
      subject: `[Project Waifu] Security Alert: Account Locked - ${userEmail}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; padding: 20px; border: 1px solid #ef4444; border-radius: 12px; background: #0f172a; color: #f8fafc;">
          <h2 style="color: #ef4444; margin-top: 0;">Account Locked (Brute Force Protection)</h2>
          <p>The account <strong>${userEmail}</strong> has been locked after 10 failed login attempts.</p>
          <p>To unlock this account immediately, click the button below:</p>
          <div style="margin: 24px 0; text-align: center;">
            <a href="${unlockUrl}" style="background-color: #ef4444; color: #ffffff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold; display: inline-block;">Unlock Account Now</a>
          </div>
          <p style="font-size: 12px; color: #94a3b8; margin-top: 24px;">If you did not expect this, please check server access logs.</p>
        </div>
      `,
    });

    console.log(`[Mailer] Account locked notification sent to admin (${adminEmail}) for user ${userEmail}.`);
    return { sent: true, unlockUrl };
  } catch (err: any) {
    console.error(`[Mailer] Failed to send account locked email:`, err);
